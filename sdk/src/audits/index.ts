...
export class AuditsClient {
    ...
    public async getAudits(params: { tenantId: string, filter?: string, sortBy?: string, sortDirection?: string, offset: number, count: number, filters: any }) {
    ...
    public async exportPdf(params: { tenantId: string, filter?: string, sortBy?: string, sortDirection?: string, filters: any }, properties: any[]) {
        Logger.info('going to export audits to pdf');
        const paramsToSend = { ...params, ...params.filters };
        delete paramsToSend.filters;

        await this.authenticator.validateAuthentication();
        const response = await axios.post(`${config.urls.auditsService}/export/pdf`, { properties }, {
            params: paramsToSend,
            responseType: 'blob',
            headers: {
                'x-access-token': this.authenticator.accessToken,
    ...
    public async exportCsv(params: { tenantId: string, filter?: string, sortBy?: string, sortDirection?: string, filters: any }, properties: any[]) {
        Logger.info('going to export audits to pdf');
        const paramsToSend = { ...params, ...params.filters };
        delete paramsToSend.filters;

        await this.authenticator.validateAuthentication();
        const response = await axios.post(`${config.urls.auditsService}/export/csv`, { properties }, {
            params: paramsToSend,
            responseType: 'blob',
            headers: {
                'x-access-token': this.authenticator.accessToken,
