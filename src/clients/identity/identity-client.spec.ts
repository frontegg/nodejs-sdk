...
import { AuthHeaderType, IEntityWithRoles, ITenantAccessToken, IUser, IUserAccessToken, TEntityWithRoles, tokenTypes } from './types';
...
describe('Identity client', () => {
    ...
    beforeEach(() => {
        ...
        axiosMock.onGet(`${config.urls.identityService}/resources/configurations/v1`, {},).reply(200, { publicKey: 'fake-public-key' });
    });
    ...
    it('should throw error if could not find resolver', async () => {
        try {
            //@ts-ignore
            await IdentityClient.getInstance().validateIdentityOnToken('fake-token', {}, 'invalid-header-type')
        } catch (e: any) {
    ...
    it('should validate entity', async () => {
        ...
        const res = await IdentityClient.getInstance().validateIdentityOnToken('fake-token', {}, AuthHeaderType.JWT)
        expect(res).toEqual(fakeUser)
    });

    it.each([
        { claims: fakeUserAccessToken, },
        { claims: fakeTenantAccessToken }
    ])('should validate access token entity without fetching roles', async ({ claims }) => {
        //@ts-ignore
        ...
        const res = await IdentityClient.getInstance().validateIdentityOnToken('fake-token', {}, AuthHeaderType.AccessToken)
    ...
    it.each([
        ...
        const res = await IdentityClient.getInstance().validateIdentityOnToken('fake-token', { roles: ['ReadOnly'] }, AuthHeaderType.AccessToken)
    ...
    it.each([
        ...
        try {
            await IdentityClient.getInstance().validateIdentityOnToken('fake-token', {}, type)
        } catch (e: any) {
    ...
    it('should throw if entity doesnt have the required roles', async () => {
        ...
        jest.spyOn(authorizationHeaderResolver, 'verifyAsync').mockImplementation(() => fakeUser)
        try {
            await IdentityClient.getInstance().validateIdentityOnToken('fake-token', { roles: ['ReadOnly'] }, AuthHeaderType.JWT)
        } catch (e: any) {
