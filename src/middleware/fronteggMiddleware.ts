...
async function proxyRequest(req, res, context, authenticator) {
  ...
  Logger.log(
    `going to proxy request - ${req.originalUrl} to ${target}`
  );
  ...
  const headers = {
    ...
    'frontegg-authenticated-entity-type':
      context.authenticatedEntityType,
  };

  if (context.userPermissions) {
    headers['frontegg-user-permissions'] =
...
export function frontegg(options: IFronteggOptions) {
  ...
  let authInitedPromise = authenticator.init(
    options.clientId,
    options.apiKey
  );
    ...
    if (options.cookieDomainRewrite) {
      ...
      Object.keys(proxyRes.headers).forEach((key) => {
        if (key.toLowerCase() === 'set-cookie') {
          proxyRes.headers[key] = rewriteCookieDomain(
  ...
  proxy.on('proxyReq', (proxyReq, req: any, res, _) => {
    try {
      ...
      if (req.body) {
        let contentType = proxyReq.getHeader(
          'Content-Type'
        ) as string;
        let contentLength = proxyReq.getHeader(
          'Content-Length'
        ) as number;
        if (
          contentType &&
  ...
  return async (req, res) => {
    try {
      ...
      authInitedPromise = authenticator.init(
        options.clientId,
        options.apiKey
      );
      throw e;
    ...
    if (
      ...
      !(await fronteggRoutes.isFronteggPublicRoute(req))
    ) {
      ...
      try {
        ...
        if (res.headersSent) {
          // response was already sent from the middleware, we have nothing left to do
          Logger.debug(
            'Headers was already sent from authMiddleware'
          );
          return;
        }
      } catch (e) {
        ...
        if (res.headersSent) {
          // response was already sent from the middleware, we have nothing left to do
          Logger.debug(
            'authMiddleware threw error, but headers was already sent'
          );
          return;
        }
    ...
    if (req.method === 'OPTIONS') {
      ...
      console.log(
        'OPTIONS call to frontegg middleware - returning STATUS 204'
      );
      res.status(204).send();
      return;
    }
